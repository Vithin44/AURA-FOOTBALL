...
// ====================================================
// 1. TELA DE PARTIDAS (MATCH PAGE)
// ====================================================
export function MatchPage() {
  const { career } = useCareer();
  const { navigateTo } = useNavigation();
  const [matchState, setMatchState] = useState<'pre_match' | 'simulating' | 'finished'>('pre_match');
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-24">
      <div className="flex items-center justify-between pb-3 border-b border-[#191C1C]">
        <div>
          <h1 className="text-xl font-bold text-[#F4F5F2]">Confronto da Rodada</h1>
          <p className="text-xs text-[#8B918E]">
            {career.club.league} • Semana {career.season.currentWeek} de {career.season.totalWeeks}
          </p>
        </div>
        <Badge variant="green">Partida Oficial</Badge>
      </div>
      ...
      <Card variant="dark" className="p-6">
        <div className="grid grid-cols-3 items-center text-center py-6">
          <div className="space-y-3">
            <div
              className="w-16 h-16 rounded-2xl mx-auto flex items-center justify-center font-display font-black text-2xl border border-white/20 shadow-lg"
              style={{
            ...
            <div>
              <h2 className="text-base font-bold text-[#F4F5F2]">{career.club.name}</h2>
              <span className="text-xs font-mono text-[#8B918E]">OVR {career.club.ovr}</span>
            </div>
          </div>

          <div className="space-y-2">
            <span className="font-mono text-xs text-[#8B918E] uppercase tracking-wider block">
              {matchState === 'simulating' ? `${simMinute}' EM ANDAMENTO` : 'PLACAR'}
            </span>
            <div className="font-mono text-3xl font-black text-[#F4F5F2] tracking-wider">
              {matchState === 'pre_match' ? 'VS' : matchState === 'simulating' ? '1 - 0' : '2 - 1'}
            </div>
            <span className="text-xs text-[#8B918E] block">{career.club.stadium}</span>
          </div>

          <div className="space-y-3">
            <div className="w-16 h-16 rounded-2xl mx-auto flex items-center justify-center font-display font-black text-2xl bg-[#3B82F6] text-[#F4F5F2] border border-white/20 shadow-lg">
              MET
            </div>
            <div>
              <h2 className="text-base font-bold text-[#F4F5F2]">Metropolitano</h2>
              <span className="text-xs font-mono text-[#8B918E]">OVR 74</span>
            </div>
          </div>
        </div>

        <div className="mt-6 pt-4 border-t border-[#191C1C] flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xs text-[#8B918E]">AURA de Início:</span>
            <Badge variant="green">{career.player.aura}% AURA</Badge>
          </div>

          {matchState === 'pre_match' && (
            <Button variant="primary" size="lg" onClick={startSimulation}>
              <Play size={16} />
              Iniciar Partida Acelerada
            </Button>
          )}

          {matchState === 'simulating' && (
            <div className="flex items-center gap-2 text-xs font-mono text-[#B7FF3C] animate-pulse">
              Simulando lances do jogo... ({simMinute}')
            </div>
          )}

          {matchState === 'finished' && (
            <div className="flex gap-2">
              <Badge variant="green" className="py-2 px-3">
                Vitória por 2x1! +1 Gol marcado
              </Badge>
              <Button variant="secondary" size="md" onClick={() => setMatchState('pre_match')}>
                Concluir Rodada
              </Button>
            </div>
          )}
